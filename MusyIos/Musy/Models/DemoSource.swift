...
class DemoSource: NSObject {
    ...
    override init() {
        ...
        Alamofire.request(connexion.url+"/getvideo").responseJSON{
            response in
            var TvShows:NSArray =  []
            TvShows = response.result.value as! NSArray
            
            for a in TvShows {
               let vid = a as! Dictionary<String,Any>
                let url = vid["video_path"] as! String
                let u = URL(string: url)
                print(u!)
                self.demoData += [DataObj(image: UIImage(named: "NakfH4"), play_Url: u! , title: "title", content: "no content")]
                
           /*   let dataobj = DataObj(image: UIImage(named: "man"), play_Url: u! , title: "title", content: "no content")
              self.demoData.append(dataobj)*/
               
            }
            print ("number of demo \(self.demoData.count)")
     ...
     /*  demoData += [
            DataObj(image: #imageLiteral(resourceName: "facebook"),
                    play_Url: URL(string: "http://192.168.1.13:3003/videos/yasmine.mp4")!,
